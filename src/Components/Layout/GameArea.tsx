...
const HeaderGame = () => {
    ...
    return (
        <Card>
            ...
            <CardContent>
                <Conteiner className="w-full flex items-center justify-between">
                    <Conteiner className="flex w-full gap-2 items-center justify-start">
                        <Avatar>
                            <AvatarFallback>{gameOptions?.name.split("")[0]}</AvatarFallback>
                        </Avatar>
                        <Conteiner className="w-auto">
                            <PText>{gameOptions?.name}</PText>
                            <PText className="text-xs text-zinc-400">Jogadas: {players.player1Jogadas}</PText>
                        </Conteiner>
                    </Conteiner>
                    <Conteiner className="flex w-full gap-2 items-center justify-center">
                        <PText className="text-4xl text-zinc-50">{players.player1Wins}</PText>
                    </Conteiner>
                    <Conteiner className="flex w-full gap-2 items-center justify-center">
                        <PText className="text-4xl text-zinc-50">{players.player2Wins}</PText>
                    </Conteiner>
                    <Conteiner className="flex w-full gap-2 items-center justify-end">
                        <Avatar>
                            <AvatarImage src="https://github.com/shadcn.png" />
                            <AvatarFallback>RD</AvatarFallback>
                        </Avatar>
                        <Conteiner className="w-auto">
                            <PText>Jogador 2</PText>
                            <PText className="text-xs text-zinc-400">Jogadas: {players.player2Jogadas}</PText>
                        </Conteiner>
                    </Conteiner>
                </Conteiner>
            </CardContent>
        ...
        <Card>
            <div className="p-6 grid grid-cols-7 gap-4">
                {columns?.map((column, colIndex) => (
                    <button
                        key={colIndex}
                        className="p-2 py-3 space-y-4 rounded-xl hover:bg-zinc-400/20 duration-150"
                        onClick={() => handleButtonClick(colIndex)}
                    >
                        {column.map((state, lineIndex) => (
                            <Celula
                                key={`${colIndex}-${lineIndex}`}
                                selectedState={{ seletion: state }}
                            />
                        ))}
                    </button>
                ))}
